...
10. **Research subagent output is not trusted output.** When a subagent reports "not found" or "data gap," verify independently before accepting. The subagent may have searched the wrong URL, been blocked by JavaScript rendering, or simply not looked hard enough. A single additional web search often finds what the subagent missed. Never accept "could not verify" without trying yourself.
11. **Follow the research plan.** When a research plan exists (own-your-mind-research-plan.md or any project-specific plan), follow it step by step. Do not skip steps, compress research, or delegate research to a subagent without verifying the output meets the plan's requirements. The plan exists because previous research was inadequate without it.
