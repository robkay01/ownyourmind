...
<div class="grid grid-cols-3 gap-3 mb-8">
  <MetricCard value="$3.15M" label="Annual revenue (on-chain)" />
  <MetricCard value="~2.1M" label="AKT monthly burn (projected)" />
  <MetricCard value="5.2%" label="Effective inflation (projected)" />
</div>

## What is happening

On 23 March 2026 at 14:00 UTC, Akash Network activates AEP-76: Burn-Mint Equilibrium. The on-chain governance vote runs 7–14 March. If it passes (and every indication is that it will, as major validators including Polkachu have already voted yes), mainnet upgrades to v2.0.0 with BME and WASM smart contract support.
...
<VerificationFunnel
  title="BME worked example: $1,000 compute at AKT $1.14→$1.50"
  steps={[
    { label: "AKT burned (tenant)", value: "877.19", source: "Mint phase at $1.14", width: 100 },
    { label: "ACT minted", value: "1,000", source: "Compute credit (USD-pegged)", width: 100 },
    { label: "AKT reminted (provider)", value: "666.67", source: "Settlement at $1.50", width: 76 },
    { label: "Net burn", value: "210.53 AKT", source: "Permanently removed", width: 24 },
  ]}
/>
...
<ComparisonTable
  title="Akash vs Render BME comparison"
  headers={["", "Akash (AEP-76)", "Render (RNP-001/006)"]}
  rows={[
    { label: "Burn trigger", values: ["Tenant top-up burns AKT to mint ACT", "Job completion burns RENDER paid by creators"] },
    { label: "Provider payment", values: ["Reminted AKT at settlement TWAP", "Separate emission pool (fixed declining schedule)"] },
    { label: "Intermediate token", values: ["ACT (soulbound, USD-pegged)", "None. RENDER burned directly"] },
    { label: "Deflationary when", values: ["AKT appreciates between mint and settle", "Burns exceed fixed emission rate"] },
    { label: "Safety mechanism", values: ["BME vault + circuit breakers at CR 0.90/0.95", "No equivalent vault described"] },
    { label: "Fee structure", values: ["25bps mint spread", "5% service fee"] },
    { label: "Supply cap", values: ["No cap (inflation + BME)", "644M hard cap"] },
    { label: "Current gap to deflation", values: ["Inflation drops from 8% to ~5.2% (Akash projection)", "Emissions outpace burns ~8x"] },
  ]}
/>
...
2. **BME vault collateral ratio.** Should stay well above 1.0. If it approaches 0.95, the market is testing the mechanism. Below 0.90 and circuit breakers halt new ACT mints, which would be a stress scenario.
