...
description: "AI agents can think, write, and code. But they cannot pay for anything. The infrastructure to change that is shipping, and the gap between what is real and what is narrative is worth understanding."
...
$1.6 million is not a big number. But it is $1.6 million more than was being spent before x402 existed, and the infrastructure being built around it matters more than the current volume. The protocol has 75+ service endpoints. The SDK is available in TypeScript, Python, and Go. Apache 2.0 licensed. The backers are not betting on current volume. They are betting on what the number looks like when agents become the default buyer.
...
It is framework-agnostic. Works with LangChain, OpenAI Agents SDK, Vercel AI, and [ElizaOS](/projects/elizaos/). Wallet-agnostic too, supporting Coinbase, Privy, and Viem wallets. 1,146 GitHub stars, 270 versions published over 13 months. This is actively maintained production software, not a hackathon project.
...
The projects that understand this will build sustainable agent economies. Use the native token for staking, governance, and network coordination. Settle agent payments in stablecoins. The projects that force agents to transact in volatile tokens will not survive.
