...
- **MetaMask** or compatible Web3 wallet, connected to Base network
- **Small amount of ETH on Base** for gas fees (~$0.01-0.05 per transaction)
- **100 VIRTUAL tokens** (~$65 at current prices) to create the agent
...
At current VIRTUAL prices (~$0.65), the graduation threshold represents roughly $27,300 in cumulative trading. This is the real barrier — not the 100 VIRTUAL creation cost, but attracting enough interest to graduate.
...
**Creation cost:** 100 VIRTUAL (~$65). Low barrier.

**Graduation cost:** You do not pay this directly — it comes from community trading. But your agent needs to attract ~$27,300 in cumulative trading to graduate. If it does not, it sits on fun.virtuals.io indefinitely.
...
**VIRTUAL token context:** Currently ~$0.65, down 87% from its all-time high of $5.07. The token has a fixed 1 billion supply with no future inflation, but whale concentration is reportedly 90%+.
...
- **AIXBT:** Market intelligence agent monitoring 400+ KOLs. Hit $500M market cap at peak.
