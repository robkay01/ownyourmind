...
Season 1 distributed 100 million GRASS (10% of supply) to approximately 2 million users in October 2024. If you started early and ran consistently, you might have received 2,000-3,000 GRASS tokens worth $500-800 at listing. At current prices (~$0.20), those same tokens are worth $400-600.

Season 2 will distribute 170 million GRASS — nearly double Season 1 — expected in H1 2026. But with 3+ million active nodes now competing for that allocation, individual shares may not increase proportionally.
...
| Circulating | ~475M |
| Price | ~$0.20 (down ~95% from ATH of $3.67) |
| Chain | Solana |
...
The tokenomics allocation is worth noting: 25.2% to early investors (1-year cliff + 1-year vest), 22% to contributors (1-year cliff + 3-year vest), 22.8% to foundation/ecosystem. Nearly half the supply is allocated to insiders. A 55 million token unlock happened on 28 February 2026.
...
The network currently processes over 100 terabytes of data daily. With 2.5 million daily active users, Grass claims it can scrape enough data to train a GPT-3.5-class model from scratch. Enterprise customers pay in GRASS tokens to access this data via APIs.
...
**Earnings are unpredictable.** There is no fixed conversion rate from points to tokens. Seasonal airdrops mean you wait months to receive anything. The token is down 95% from its all-time high.
